...
export class BookUploadService {
	...
	async uploadCover(
		...
		const book = await this.bookRepository.findById(bookId, ['covers']);
	...
	async replaceCoverImage(
		...
		const cover = await this.coverRepository.findById(coverId, ['book']);

		if (!cover || cover.book.id !== bookId) {
			this.logger.warn(
	...
	async uploadMultipleCovers(
		...
		try {
			...
			const savedCovers = await this.coverRepository.saveAll(covers);
	...
	async uploadChapterPages(
		...
		try {
			...
			const savedPages = await this.pageRepository.saveAll(pages);
