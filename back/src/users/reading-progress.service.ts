...
export class ReadingProgressService {
     ...
     */
    async getAllProgress(userId: string): Promise<ReadingProgressResponseDto[]> {
        const progressList = await this.progressRepository.find({
            where: { userId },
            order: { updatedAt: 'DESC' },
        });
    ...
    async deleteProgress(userId: string, chapterId: string): Promise<void> {
        ...
        this.logger.debug(`Progresso removido: user=${userId}, chapter=${chapterId}`);
    }
    ...
    async deleteBookProgress(userId: string, bookId: string): Promise<void> {
        ...
        this.logger.debug(`Progresso do livro removido: user=${userId}, book=${bookId}`);
    }

    private toResponseDto(progress: ReadingProgress): ReadingProgressResponseDto {
        return {
            id: progress.id,
