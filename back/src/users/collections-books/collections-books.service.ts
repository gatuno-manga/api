...
export class CollectionsBooksService {
	...
	async addBookToCollection(
		...
		if (books.length !== dto.idsBook.length) {
			...
			const notFoundIds = dto.idsBook.filter(
				(id) => !foundIds.includes(id),
			);
			throw new NotFoundException(
				`Books not found: ${notFoundIds.join(', ')}`,
			);
		}
		...
		if (newBooks.length === 0) {
			throw new BadRequestException(
				'All books are already in the collection',
			);
		}

		collection.books = [...collection.books, ...newBooks];
		const savedCollection =
			await this.collectionBookRepository.save(collection);
