...
export class SavedPagesService {
     ...
     */
    async savePage(dto: CreateSavedPageDto, userId: string): Promise<SavedPage> {
        ...
        if (page.chapter.id !== dto.chapterId) {
            throw new BadRequestException('Page does not belong to the specified chapter');
        }
     ...
     */
    async updateComment(id: string, dto: UpdateSavedPageDto, userId: string): Promise<SavedPage> {
     ...
     */
    async countSavedPagesByBook(userId: string, bookId: string): Promise<number> {
