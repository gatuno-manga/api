...
export class SavedPagesController {
    ...
    @ApiResponse({ status: 400, description: 'Page already saved or invalid data' })
    ...
    @ApiResponse({ status: 200, description: 'Saved pages retrieved successfully' })
    @ApiResponse({ status: 401, description: 'Unauthorized' })
    ...
    async getSavedPagesByChapter(
        ...
        return this.savedPagesService.getSavedPagesByChapter(user.userId, chapterId);
    }
    ...
    @ApiResponse({ status: 200, description: 'Returns whether the page is saved' })
    @ApiResponse({ status: 401, description: 'Unauthorized' })
    async isPageSaved(
        @Param('pageId', ParseIntPipe) pageId: number,
        @CurrentUser() user: CurrentUserDto,
    ) {
        const isSaved = await this.savedPagesService.isPageSaved(pageId, user.userId);
    ...
    async countSavedPagesByBook(
        ...
        const count = await this.savedPagesService.countSavedPagesByBook(user.userId, bookId);
        return { bookId, count };
    }
    ...
    @ApiOperation({
        ...
        description: 'Remove a page from saved pages using the original page ID',
    })
    @ApiParam({
        name: 'pageId',
