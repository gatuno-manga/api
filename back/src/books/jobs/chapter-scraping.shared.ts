...
export class ChapterScrapingSharedService {
     ...
     */
    async processChapterPages(chapter: Chapter, minPages?: number): Promise<boolean> {
        ...
        try {
            ...
            // Faz o scraping
            const pages = await this.scrapingService.scrapePages(chapter.originalUrl, minPages);

            if (!pages || pages.length === 0) {
                chapter.scrapingStatus = ScrapingStatus.ERROR;
                await this.chapterRepository.save(chapter);
                this.logger.warn(`Nenhuma página encontrada para o capítulo: ${chapterInfo}`);
            ...
            this.logger.log(`Páginas salvas para o capítulo: ${chapterInfo} em ${(endTime - startTime) / 1000}s`);
