...
export class BookContentUpdateService {
    ...
    private async getExistingChapterIndexes(
        ...
        return new Set([
            ...existingIndexesResult.map((r) => parseFloat(r.index)),
    ...
    private async syncCovers(
        ...
        for (const scrapedCover of scrapedCovers) {
            try {
                const imageHash = await this.coverImageService.calculateImageHash(
                    scrapedCover.url,
                    refererUrl,
                );
                ...
                await this.coverImageService.addCoverToQueue(book.id, refererUrl, [
