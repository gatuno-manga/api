...
export class BookRelationshipService {
     ...
     */
    async findOrCreateAuthors(authorsDto: CreateAuthorDto[]): Promise<Author[]> {
        return Promise.all(
     ...
     */
    async findOrCreateSensitiveContent(sensitiveContentNames: string[]): Promise<SensitiveContent[]> {
        return Promise.all(
