...
export class FileDeletionEvents {
    ...
    async handleBookDeleted(event: {
        ...
        this.logger.log(`Processing deletion of book files: ${event.bookTitle}`);
        ...
        for (const coverPath of event.covers) {
            try {
                ...
                this.logger.error(`Failed to delete cover file ${coverPath}:`, error);
                errorCount++;
            }
        }
        ...
        for (const pagePath of event.pages) {
            try {
                ...
                this.logger.error(`Failed to delete page file ${pagePath}:`, error);
                errorCount++;
            }
        }
    ...
    async handleChapterDeleted(event: {
        ...
        this.logger.log(`Processing deletion of chapter files: ${event.chapterId}`);
    ...
    async handleCoverDeleted(event: { coverId: string; url: string }) {
        ...
        try {
            ...
            this.logger.error(`Failed to delete cover file ${event.url}:`, error);
        }
    }
    ...
    async handlePageDeleted(event: {
        ...
        try {
            ...
            this.logger.error(`Failed to delete page file ${event.path}:`, error);
        }
    }
}
