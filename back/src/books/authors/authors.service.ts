...
export class AuthorsService {
    ...
    async get(options: AuthorsOptions, maxWeightSensitiveContent: number = 99): Promise<Author[]> {
    ...
    async getAll(options: AuthorsOptions, maxWeightSensitiveContent: number = 99): Promise<Author[]> {
    ...
    async mergeAuthors(id: string, copy: string[]) {
        ...
        if (copyAuthors.length === 0) {
            ...
            throw new NotFoundException(`No authors found for ids: ${copy.join(', ')}`);
        }
