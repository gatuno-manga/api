...
export class DashboardService {
  ...
  async getOverview() {
    ...
    const totalSensitiveContent = await this.sensitiveContentRepository.count();
    ...
    return {
      ...
      status: {
        books: booksByStatus.map(item => ({ status: item.status || 'UNKNOWN', count: parseInt(item.count) })),
        chapters: chaptersByStatus.map(item => ({ status: item.status || 'UNKNOWN', count: parseInt(item.count) })),
      },
      sensitiveContent: sensitiveContentDistribution.map(item => ({ name: item.name, count: parseInt(item.count) })),
      tags: tagsDistribution.map(item => ({ name: item.name, count: parseInt(item.count) }))
    };
  }
}
