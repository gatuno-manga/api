...
export class FileCleanupCron {
    ...
    constructor(
        ...
        this.enabled = this.configService.get<string>('FILE_CLEANUP_ENABLED', 'true') === 'true';
    }
    ...
    async dailyCleanupOldDeletedFiles() {
        ...
        try {
            const report = await this.fileCleanupService.cleanupOldDeletedFiles();
    ...
    async weeklyIntegrityCheck() {
        ...
        try {
            ...
            if (missingFiles.length > 0) {
                ...
                this.logger.error(`Missing files by type: ${JSON.stringify(byType)}`);
            }
        } catch (error) {
