...
export class FileCleanupService {
    ...
    constructor(
        ...
        this.retentionDays = this.configService.get<number>('SOFT_DELETE_RETENTION_DAYS', 10);
    }
    ...
    async findOrphanFiles(): Promise<OrphanFile[]> {
        ...
        try {
            ...
            const pageFiles = new Set(pages.map(p => p.path.replace('/data/', '')));
            const coverFiles = new Set(covers.map(c => c.url.replace('/data/', '')));
            ...
            for (const filename of files) {
                if (!allReferencedFiles.has(filename)) {
                    ...
                    try {
                        ...
                        if (stats.isDirectory()) {
                            this.logger.debug(`Skipping directory: ${filename}`);
                            continue;
                        }
                        ...
                        this.logger.error(`Error reading file stats for ${filename}:`, error);
                    }
                }
            }
    ...
    async cleanupOrphanFiles(dryRun: boolean = true): Promise<CleanupReport> {
        ...
        this.logger.log(`Cleanup completed: ${report.filesDeleted} files deleted, ${(report.spaceRecovered / 1024 / 1024).toFixed(2)} MB recovered`);
        return report;
    }
    ...
    async cleanupOldDeletedFiles(): Promise<CleanupReport> {
        this.logger.log(`Cleaning up files deleted more than ${this.retentionDays} days ago...`);
    ...
    async getStorageStatistics(): Promise<any> {
        ...
        const deletedPages = await this.pageRepository.count({ withDeleted: true }) - pages;
        const deletedCovers = await this.coverRepository.count({ withDeleted: true }) - covers;
