...
describe('RedisConcurrencyManager', () => {
    ...
    describe('acquire', () => {
        it('should allow unlimited access when limit is null', async () => {
            await expect(manager.acquire('example.com', null)).resolves.toBeUndefined();
            expect(mockRedis.eval).not.toHaveBeenCalled();
        });

        it('should allow unlimited access when limit is 0', async () => {
            await expect(manager.acquire('example.com', 0)).resolves.toBeUndefined();
            expect(mockRedis.eval).not.toHaveBeenCalled();
        });

        it('should allow unlimited access when limit is negative', async () => {
            await expect(manager.acquire('example.com', -1)).resolves.toBeUndefined();
            expect(mockRedis.eval).not.toHaveBeenCalled();
        });
