...
describe('InMemoryConcurrencyManager', () => {
    ...
    describe('acquire and release', () => {
        it('should allow unlimited access when limit is null', async () => {
            ...
            await expect(manager.acquire(domain, null)).resolves.toBeUndefined();
            await expect(manager.acquire(domain, null)).resolves.toBeUndefined();
            await expect(manager.acquire(domain, null)).resolves.toBeUndefined();
        });
        ...
        it('should allow acquire up to limit', async () => {
            ...
            await expect(manager.acquire(domain, limit)).resolves.toBeUndefined();
            await expect(manager.acquire(domain, limit)).resolves.toBeUndefined();
            await expect(manager.acquire(domain, limit)).resolves.toBeUndefined();
        });
        ...
        it('should handle multiple domains independently', async () => {
            ...
            await expect(manager.acquire(domain2, limit)).resolves.toBeUndefined();
        ...
        it('should maintain FIFO queue order', async () => {
            ...
            const promise1 = manager.acquire(domain, limit).then(() => order.push(1));
            const promise2 = manager.acquire(domain, limit).then(() => order.push(2));
            const promise3 = manager.acquire(domain, limit).then(() => order.push(3));
