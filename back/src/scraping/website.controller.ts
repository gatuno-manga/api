...
export class WebsiteController {
	...
	@Get()
	@Roles(RolesEnum.ADMIN)
	@ApiOperation({ summary: 'List all websites', description: 'Get all registered website configurations (Admin only)' })
	@ApiResponse({ status: 200, description: 'List of websites returned successfully' })
	@ApiResponse({ status: 401, description: 'Unauthorized' })
	@ApiResponse({ status: 403, description: 'Forbidden - Admin role required' })
	async findAll() {
	...
	@Get(':id')
	@Roles(RolesEnum.ADMIN)
	@ApiOperation({ summary: 'Get website by ID', description: 'Get a specific website configuration by ID (Admin only)' })
	...
	@Roles(RolesEnum.ADMIN)
	@ApiOperation({ summary: 'Update website', description: 'Update an existing website configuration (Admin only)' })
	@ApiParam({ name: 'id', description: 'Website UUID', type: 'string' })
	...
	@Throttle({ short: { limit: 5, ttl: 300000 } }) // 5 req/5min
	@Roles(RolesEnum.ADMIN)
	@ApiOperation({ summary: 'Delete website', description: 'Delete a website configuration (Admin only)' })
	@ApiParam({ name: 'id', description: 'Website UUID', type: 'string' })
