...
export class StorageInjector {
	...
	async injectCookies(
		...
		const playwrightCookies = cookies.map((cookie) => ({
			...
			expires:
				cookie.expires || Math.floor(Date.now() / 1000) + 86400 * 365, // Default: 1 year
		}));

		await context.addCookies(playwrightCookies);
		this.logger.debug(
			`Injected ${cookies.length} cookies for domain: ${defaultDomain}`,
		);
	}
	...
	async injectLocalStorage(page: Page): Promise<void> {
		...
		this.logger.debug(
			`Injected ${Object.keys(localStorage).length} localStorage items`,
		);
	}
	...
	async injectSessionStorage(page: Page): Promise<void> {
		...
		this.logger.debug(
			`Injected ${Object.keys(sessionStorage).length} sessionStorage items`,
		);
	}
	...
	async injectStorageAndReload(page: Page): Promise<boolean> {
		...
		if (
			...
			Object.keys(this.config.localStorage).length > 0
		) {
			...
			this.logger.debug(
				`localStorage verification: ${JSON.stringify(actualValues)}`,
			);
		}
		...
		const hasStorageItems =
			...
			(this.config.sessionStorage &&
				Object.keys(this.config.sessionStorage).length > 0);
